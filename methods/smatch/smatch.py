...
def get_best_match(instance1, attribute1, relation1,
    ...
    # calculating problem size in log to avoid numpy overflow
    log10_search_space = np.sum([np.log10(len(mapping)) for mapping in candidate_mappings if mapping])
    ...
    return best_mapping, best_match_num, best_iteration, log10_search_space, runtime
    ...
    if dorelation:
        for relation1_item in relation1:
            for relation2_item in relation2:
                # if both relation share the same name, and the source/target instance share the same name
...
def get_amr_match(cur_amr1, cur_amr2, sent_num=1, justinstance=False, justattribute=False, justrelation=False):
    ...
    (best_mapping, best_match_num, best_iteration, log10_search_space, runtime) = get_best_match(instance1, attributes1, relation1,
    ...
    return best_match_num, test_triple_num, gold_triple_num, instance1, instance2, attributes1, attributes2, relation1, relation2, best_iteration, log10_search_space, runtime
...
def score_amr_pairs(f1, f2, justinstance=False, justattribute=False, justrelation=False):
    ...
    for sent_num, (cur_amr1, cur_amr2) in enumerate(generate_amr_lines(f1, f2), start=1):
        best_match_num, test_triple_num, gold_triple_num, instance1, instance2, attributes1, attributes2, relation1, relation2, best_iteration, log10_search_space, runtime = get_amr_match(cur_amr1, cur_amr2,
        ...
        if not single_score:  # if each AMR pair should have a score, compute and output it here
            yield compute_f(best_match_num, test_triple_num, gold_triple_num), runtime, instance1, instance2, attributes1, attributes2, relation1, relation2, best_iteration, log10_search_space
    if verbose:
    ...
    if single_score:  # output document-level smatch score (a single f-score for all AMR pairs in two files)
        yield compute_f(total_match_num, total_test_num, total_gold_num), None, None, None, None, None, None, None, None, None
...
def main(args):
    ...
    f0 = open(args.f[0], 'r')
    f1 = open(args.f[1], 'r')
    for (precision, recall, best_f_score), runtime, instance1, instance2, attributes1, attributes2, relation1, relation2, best_iteration, log10_search_space in score_amr_pairs(f0, f1,
        ...
        search_spaces.append(10**log10_search_space)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-f',
        nargs=2,
        default=['/N/scratch/zkachwal/amr3_data/unoisy1_gold_unwiki.txt','/N/scratch/zkachwal/amr3_data/noisy1_gold_unwiki.txt'],
        type=str,
